...
function CommentForm(): JSX.Element {
  ...
  const handleFieldChange = (evt: {
    target: { name: string; value: NewComment[keyof NewComment] };
  }) => {
  ...
  return (
    <form className="reviews__form form" method="post" action="#todo">
      ...
      <textarea className="reviews__textarea form__textarea" id="comment" name="comment"
        ...
        onChange={handleFieldChange} value={formData.comment}
      />
      <div className="reviews__button-wrapper">
        <p className="reviews__help">
          To submit review please make sure to set <span className="reviews__star">rating</span> and describe your stay with at least <b className="reviews__text-amount">50 characters</b>.
        </p>
        <button className="reviews__submit form__submit button" type="submit" disabled={(formData.rating === 0) || (formData.comment.length < 50)}>Submit</button>
      </div>
    </form>
  );
}
